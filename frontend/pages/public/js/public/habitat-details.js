document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchHabitats() {
        try {
            const res = await fetch('http://localhost:2024/api/habitats');
    ...
    async function fetchAnimals(habitatId) {
        try {
            const res = await fetch('http://localhost:2024/api/animals');
          ...
          `;
                ...
                card.querySelector('button').addEventListener('click', () => {
                    fetch(`http://localhost:2024/api/statistiques/increment/${animal.id}`, {
                        method: 'POST',
    ...
    async function fetchRapports(animalId) {
        try {
            const res = await fetch(`http://localhost:2024/api/rapports-veterinaires/${animalId}`);
